...
Pull the published image and run it with your config and AWS credentials mounted in. The image
bundles the plugin and a matching Fluent Bit version, so nothing else is needed. Images are
published for `linux/amd64`.

Download the sample config to edit (or supply your own):
  ```shell
...
Builds the plugin and image from source in this repo:
  ```shell
  docker compose up
  ```
